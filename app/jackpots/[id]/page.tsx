...
export default function JackpotDetailPage() {
  ...
  const isEnded = new Date(mockJackpot.endDate).getTime() <= Date.now();
            ...
            {activeTab === 'leaderboard' && (
              <div className="space-y-4">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className={`flex items-center space-x-2 ${isEnded ? 'text-gray-400' : 'text-green-400'}`}>
                    {isEnded ? (
                      <PauseIcon className="w-4 h-4" />
                    ) : (
                      <PlayIcon className="w-4 h-4" />
                    )}
                    <span className="text-sm font-medium">{isEnded ? 'Final' : 'Live'}</span>
                  </div>
                </div>
                ...
                <div className="card">
                  ...
                  {isEnded ? (
                    ...
                    <p className="text-sm text-gray-400">
                      Exact split is randomized and revealed after the campaign ends to keep things fair.
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="card">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  {isEnded ? (
                    <span className="px-2 py-1 bg-gray-500/10 text-gray-300 text-sm rounded">Completed</span>
                  ) : (
                    <span className="px-2 py-1 bg-green-400/10 text-green-400 text-sm rounded">Active</span>
                  )}
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-gray-400">{isEnded ? 'Ended' : 'Ends In'}</span>
                  <span className="text-white font-semibold">{isEnded ? new Date(mockJackpot.endDate).toLocaleDateString() : `${daysLeft} days`}</span>
                </div>
                <div className="flex items-center justify-between">
